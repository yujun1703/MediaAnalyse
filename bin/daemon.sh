...
usage="Usage: daemon.sh (start|stop|status) (analytics|app)"
...
case $startStop in
  ...
  (start)
    ...
    case ${command} in
      analytics )
        cd ${WOOGEEN_HOME}/analytics/
        export PATH=${WOOGEEN_HOME}/analytics/bin:$PATH
        nohup nice -n ${WOOGEEN_NICENESS} node index.js \
          > "${stdout}" 2>&1 </dev/null &
        echo $! > ${pid}
        ;;
      app )
        cd ${WOOGEEN_HOME}/analytics/
        nohup nice -n ${WOOGEEN_NICENESS} node sampleAnalyticsServer.js \
          > "${stdout}" 2>&1 </dev/null &
        echo $! > ${pid}
        ;;
      * )
