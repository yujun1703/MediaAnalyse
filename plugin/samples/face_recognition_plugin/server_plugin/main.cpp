...
vector<Rect> get_final_faces(){
    ...
    final_faces.clear();
    int num_x =max(frame.cols/300,1) ;
    int num_y =max(frame.rows/300,1) ;
    ...
    //in the boundary face restuls, form a 300x300 mat with all boundary faces, and then do facedetection once more. 
    ...
    //the boundary_face_frame is a 300x300 frame consists of every potential boundary-crossing faces
    ...
    for (auto result : FaceDetection.results){
        ...
        if (result.confidence>0.3) {
            ...
            for (auto link : links){
                ...
                {
                    //map the face detection result from 300x300 boundary-frame to origianl frame
