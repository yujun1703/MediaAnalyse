...
const runSocketIOSample = function() {
  ...
  conference.addEventListener('streamadded', (event)=>{
    ...
    if (event.stream.origin !== 'analytics-server' || (event.stream.attributes && typeof event.stream.attributes === 'object' &&
       !event.stream.attributes.hasOwnProperty('origin'))) {
        var new_token;
        //var new_myRoom = '5b726342783e9f79ff081a97';
        var new_myRoom;
        createToken(new_myRoom, 'analytics-server', 'presenter', function (response) {
          new_token = response;
          console.log('requesting analytics for stream:', event.stream.id);
          //var req_body = {access_token: new_token,  stream: event.stream.id,
          //               plugin: "b849f44bee074b08bf3e627f3fc927c7"};
          //var req_body = {access_token: new_token,  stream: event.stream.id,
          //               plugin: "3f932ff2a80341faa0a73ebb3bcfb85d"};
          //var req_body = {access_token: new_token,  stream: event.stream.id,
          //               plugin: "7e399f65545b482194d6f6ae1d46df05"};
          var req_body = {access_token: new_token,  stream: event.stream.id,
                         plugin: "dc51138a8284436f873418a21ba8cfa7"};
          send('POST', '/startAnalyticsForStream/', req_body, onResponse);
      });
    }
  ...
  window.onload = function() {
    ...
    createToken(myRoom, 'user1', 'presenter', function(response) {
      var token = response;
